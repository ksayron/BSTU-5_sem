...
void main()
{
	...
	cout << "Mailslot сервер ожидает клиентов...\n";
	...
	while (true)
	{
		BOOL ok = ReadFile(mailslot, rbuf, sizeof(rbuf), &rb, NULL);
		...
		if (!ok)
		{
			...
			{
				cout << "Тайм-аут: за 15 секунд сообщений не поступило.\n";
				continue; 
			}

			throw SetMailError("ReadFile: ", err);
		}
