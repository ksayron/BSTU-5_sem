...
int main()
{
	...
	try
	{
		...
		cout << "Enter server ip adress (127.0.0.1):";
		char addr[256];
		cin >> addr;
		SOCKADDR_IN serv;
	...
	/*		int received = recv(sC, recvBuf, strlen(recvBuf) + 1, 0);
			if (received == SOCKET_ERROR) cout<< SetErrorMsgText("recv failed", WSAGetLastError());
			if (received == 0) {
				cout << "Server closed connection (recv==0)." << endl;
				break;
			}
			std::cout << "[Server " << serv.sin_addr.s_addr << ":" << 2000 << "] → " << recvBuf << std::endl;*/
		}
