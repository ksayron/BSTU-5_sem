...
namespace HT {
    ...
    void StartSnapshotThread(HTHANDLE* ht) {
        if (ht->isSnapshotRunning) {
            std::cout << "Snap thread is already running " << std::endl;
            return;
        }
        ...
        if (ht->hSnapshotThread) {
            ...
            std::cout << "Snap thread is  running" << std::endl;
        }
        else {
            cout << "Snap thread launch error" << GetLastError() << endl;
            CloseHandle(ht->FileMapping);
            CloseHandle(ht->File);
            delete ht;
    ...
    void StopSnapshotThread(HTHANDLE* ht) {
        ...
        std::cout << "Stopping snap thread " << std::endl;
        ...
        std::cout << "Snap thread has stopped" << std::endl;
    }
