...
-- ? ??????? ? ??????? ??? ?????????
GRANT CREATE TABLESPACE TO KNP;
GRANT ALTER TABLESPACE TO KNP;
...
-- ???? ?? SYS
alter user KNP quota unlimited on t1;
alter user KNP quota unlimited on t2;
alter user KNP quota unlimited on t3;
alter user KNP quota unlimited on t4;
